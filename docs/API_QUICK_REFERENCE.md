...
Returns cohort-level learning quality proxies and uplift metrics.
...
Notes:
- Endpoint is debug-only and respects `ENABLE_DEBUG_ENDPOINTS` / environment gating.
- `after_completed` and `after_skipped` are next-outcome cohorts derived from each user's event sequence.
- `relative_like_rate_uplift` is `null` when skipper cohort like rate is `0` or unavailable.
