...
class EmbeddingCollaborativeFilter:
    ...
    def __init__(self, product_vectors: np.ndarray, product_index: Dict[str, int]):
        ...
        self.user_interaction_history: Dict[
            str, List[Tuple[int, int]]
        ] = {}  # user_id -> [(product_idx, label)]
    ...
    def build_user_embedding(self, user_id: str, use_cache: bool = True) -> np.ndarray:
        ...
        if (
            user_id not in self.user_interaction_history
            or not self.user_interaction_history[user_id]
        ):
        ...
        if norm > 1e-9:
            ...
            liked_products = [
                self.product_vectors[idx] for idx, label in interactions if label == 1
            ]
            if liked_products:
                user_embedding = np.mean(liked_products, axis=0)
                user_embedding = user_embedding / (
                    np.linalg.norm(user_embedding) + 1e-9
                )
            else:
    ...
    def find_collaborative_similar_products(
        self, user_embedding: np.ndarray, candidate_indices: List[int], top_k: int = 10
        ...
        sims = cosine_similarity(
            user_embedding.reshape(1, -1), candidate_vectors
        ).flatten()

        # Create list of (original_idx, similarity)
        results = [
            (candidate_indices[i], float(sims[i]))
    ...
    def rank_products_collaborative(
        self, user_embedding: np.ndarray, product_ids: List[str], top_n: int = 10
        ...
        ranked = self.find_collaborative_similar_products(
            user_embedding, list(indices), top_k=top_n
        )
