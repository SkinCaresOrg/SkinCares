...
class SwipeSession:
    ...
    def __init__(
        ...
        self.online_learner = OnlineLearner(
            dim=product_vectors.shape[1], learning_rate=learning_rate
        )
    ...
    def _seed_models_from_onboarding(self, skin_type: str, skin_concerns: List[str]):
        ...
        for idx, row in self.product_metadata.iterrows():
            ...
            match_count = sum(
                1 for concern in concerns_lower if concern in product_text
            )
            match_count += sum(
                1
    ...
    def get_next_product(self) -> Optional[Tuple[str, Dict]]:
        ...
        candidate_ids = [
            pid for pid in self.product_index.keys() if pid not in self.products_shown
        ]
    ...
    def record_swipe(
        ...
        self.feedback_collector.record_feedback(
            ...
            if product_id in self.product_metadata["product_id"].values
            else "Unknown",
    ...
    def get_recommendations(self, top_n: int = 5) -> pd.DataFrame:
        ...
        recommendations = recommendations.merge(
            self.product_metadata, on="product_id", how="left"
        )
