...
def _fetch(
    url: str, extra_headers: Optional[dict] = None, timeout: int = REQUEST_TIMEOUT
) -> Optional[str]:
    try:
...
def _load_checkpoint(output_path: str) -> dict[int, PriceResult]:
    ...
    try:
        ...
        for _, row in done.iterrows():
            ...
            source = (
                str(row["price_source"]) if pd.notna(row.get("price_source")) else None
            )
...
def run(input_path: str, output_path: str, workers: int = MAX_WORKERS) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            status = (
                f"${result.price:.2f} via {result.source}"
                if result.found
                else f"x {result.error}"
            )
            brand = df.at[idx, "brand"]
            name = df.at[idx, "product_name"].split(",")[0]
            log.info(
                "[%d/%d] %-46s -> %s", done_count, total, f"{brand} {name}"[:46], status
            )
